...
const isAllowHighPermission = (highPermissions, userPermissions) => highPermissions.some((per) => isAllowProperty(userPermissions, per));

const isAllowPermission = (requirePermissions, userPermissions) => requirePermissions.every((per) => isAllowProperty(userPermissions, per))
  || isEmpty(requirePermissions);

const insertParamToPlaceholder = (pers, params) => pers.map((per) => Object.entries(params).reduce(
...
const AccessFilter = (requirePers = [], highPers = []) => async (
  ...
  try {
    ...
    if (!isAllowPermission(requirePermissions, userPermissions)) { return Forbidden(res, 'Sorry you not have permission to do this action'); }
    return next();
  } catch (err) {
